...
extension CLLocation {
    func asCameraPosition() -> MapCameraPosition {
        return MapCameraPosition.camera( MapCamera( centerCoordinate: CLLocationCoordinate2D(latitude: self.coordinate.latitude, longitude: self.coordinate.longitude), distance: 10, heading: self.course))
    }
